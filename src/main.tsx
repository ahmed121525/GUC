...
function AmbientBackground(){return <div className="ambient-art" aria-hidden="true"><span className="ambient-grid"/><span className="ambient-noise"/><span className="ambient-scanline"/><span className="ambient-orbit orbit-1"/><span className="ambient-orbit orbit-2"/><span className="ambient-orbit orbit-3"/><span className="ambient-orbit orbit-4"/><span className="ambient-blob blob-1"/><span className="ambient-blob blob-2"/><span className="ambient-blob blob-3"/><span className="ambient-cross cross-1"/><span className="ambient-cross cross-2"/><span className="ambient-star star-1">✦</span><span className="ambient-star star-2">✧</span><span className="ambient-star star-3">✦</span><span className="ambient-ring-core"/><span className="ambient-diamond"/></div>}
...
function Layout({children}:{children:React.ReactNode}) {
  ...
  return <div className="app-shell"><AmbientBackground/><Particles/><CustomCursor/><ScrollParallax/><header className="site-header"><div className="wrap nav-wrap"><Link className="brand" to="/"><span className="brand-mark"><i>G</i><b>U</b><em>C</em><span className="brand-spark">✦</span></span><span className="brand-copy"><strong>GUC</strong><small>Girl Up Conquistadors</small></span><span className="brand-arrow">↗</span></Link><button className={`menu-btn ${open?'is-open':''}`} onClick={()=>setOpen(v=>!v)} aria-expanded={open} aria-label={open?'Close menu':'Open menu'}><span/><span/><span/><b className="menu-pulse"/></button><nav className={open?'nav open':'nav'}>{nav.map(([href,label])=><NavLink key={href} to={href} end={href==='/' } className={({isActive})=>isActive?'active':''}>{label}</NavLink>)}</nav><Link className="header-cta" to="/donate"><span className="cta-pulse"/> <span>Back the mission</span><b>↗</b></Link></div></header>{children}<footer className="footer"><div className="wrap footer-grid"><div><div className="footer-brand">GUC<span> / Girl Up Conquistadors</span></div><p>Youth-led action for girls who deserve access, agency, safety, and a future built on their terms.</p></div><div><h4>Navigate</h4><Link to="/about">About</Link><Link to="/events">Events</Link><Link to="/archive">Archive</Link><Link to="/join">Join Us</Link></div><div><h4>Contact</h4><a href={`mailto:${SITE.email}`}>{SITE.email}</a><a href={`tel:${SITE.phone}`}>{SITE.phone}</a><span>{SITE.locationAddress}</span></div></div><div className="wrap footer-bottom"><span>© {new Date().getFullYear()} Girl Up Conquistadors</span><span>Configure contact + donation details before launch.</span></div></footer></div>
}
...
function Archive(){usePageMeta('Archive | Girl Up Conquistadors','Browse the Girl Up Conquistadors digital archive by year, month, and event.');const [year,setYear]=useState<string|null>(null);const [month,setMonth]=useState<{year:string;month:string}|null>(null);const [event,setEvent]=useState<{year:string;month:string;event:string}|null>(null);const [lightbox,setLightbox]=useState<string|null>(null);const years=ARCHIVE;if(event){const y=years.find(v=>v.year===event.year), mo=y?.months.find(v=>v.month===event.month), ev=mo?.events.find(v=>v.id===event.event);return <main className="page"><div className="wrap page-hero compact"><span className="eyebrow">ARCHIVE / EVENT</span><h1>{ev?.name}</h1><p><button className="crumb-btn" onClick={()=>setEvent(null)}>← Back to {event.month}</button></p></div><section className="section"><div className="wrap"><div className="archive-photo-grid">{ev?.images.length?ev.images.map(img=><button key={img.id} className="archive-photo" onClick={()=>setLightbox(img.src)}><img src={img.src} alt={img.alt} loading="lazy"/><span>VIEW</span></button>):<EmptyState text="No photos have been added to this event yet."/>}</div></div></section>{lightbox&&<Modal onClose={()=>setLightbox(null)}><img className="lightbox-img" src={lightbox} alt="Archive enlargement"/></Modal>}</main>}
if(month){const y=years.find(v=>v.year===month.year),mo=y?.months.find(v=>v.month===month.month);return <main className="page"><div className="wrap page-hero compact"><span className="eyebrow">ARCHIVE / {month.year}</span><h1>{month.month}</h1><p><button className="crumb-btn" onClick={()=>setMonth(null)}>← Back to {month.year}</button></p></div><section className="section"><div className="wrap archive-folders">{mo?.events.map(ev=><button key={ev.id} className="folder-card" onClick={()=>setEvent({year:month.year,month:month.month,event:ev.id})}><span className="folder-tab"/><span className="folder-icon">▱</span><strong>{ev.name}</strong><small>{ev.images.length} image{ev.images.length===1?'':'s'}</small></button>)}{!mo?.events.length&&<EmptyState text="No events have been filed for this month."/>}</div></section></main>}
return <main className="page"><div className="wrap page-hero compact"><span className="eyebrow">ARCHIVE / MEMORY AS INFRASTRUCTURE</span><h1>Every campaign leaves a trail.</h1><p>Browse the archive like a filing cabinet from the future. Years → months → events → images.</p></div><section className="section"><div className="wrap archive-folders">{year===null?years.map(y=><button key={y.year} className="folder-card year" onClick={()=>{setYear(y.year);setMonth(null)}}><span className="folder-tab"/><span className="folder-icon">▱</span><strong>{y.year}</strong><small>{y.months.length} month{y.months.length===1?'':'s'} filed</small></button>):<><div className="archive-back"><button className="crumb-btn" onClick={()=>setYear(null)}>← All years</button></div>{years.find(y=>y.year===year)?.months.map(mo=><button key={mo.month} className="folder-card" onClick={()=>setMonth({year,month:mo.month})}><span className="folder-tab"/><span className="folder-icon">▱</span><strong>{mo.month}</strong><small>{mo.events.length} event{mo.events.length===1?'':'s'}</small></button>)}</>}</div></section><section className="section"><div className="wrap upload-panel"><div><span className="eyebrow">UPLOAD STRATEGY</span><h2>Vercel is not a filing cabinet.</h2><p>For production, store images in Supabase Storage and event metadata in Postgres. This keeps deployments immutable and image uploads independent of GitHub.</p></div><ArchiveUploader/></div></section></main>}
function EmptyState({text}:{text:string}){return <div className="empty-state"><span>∅</span><h3>Nothing filed yet</h3><p>{text}</p></div>}
function ArchiveUploader(){const [msg,setMsg]=useState('');const [busy,setBusy]=useState(false);const [files,setFiles]=useState<File[]>([]);const upload=async()=>{if(!files.length)return;setBusy(true);setMsg('');try{if(!supabaseConfigured||!supabase)throw new Error('Supabase is not configured. Add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY.');for(const file of files){if(!/image\/(jpeg|png|webp)/.test(file.type))throw new Error(`${file.name}: use JPG, PNG, or WebP.`);if(file.size>8*1024*1024)throw new Error(`${file.name}: max size is 8 MB.`);const path=`archive/${new Date().getFullYear()}/${String(new Date().getMonth()+1).padStart(2,'0')}/${crypto.randomUUID()}-${file.name}`;const {error}=await supabase.storage.from('guc-archive').upload(path,file,{cacheControl:'31536000',upsert:false});if(error)throw error;}setMsg('Upload complete. Add the returned public paths to your metadata table or admin workflow.');setFiles([]);}catch(e){setMsg(e instanceof Error?e.message:'Upload failed.');}finally{setBusy(false)}};return <div className="uploader"><label className="dropzone"><input type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={e=>setFiles(Array.from(e.target.files||[]))}/><span>DROP IMAGES OR CLICK</span><small>JPG / PNG / WEBP • 8 MB MAX</small></label>{files.length>0&&<div className="file-list">{files.map(f=><span key={f.name}>{f.name}</span>)}</div>}<button className="btn btn-primary" onClick={upload} disabled={busy||!files.length}>{busy?'Uploading…':'Upload to archive'}</button>{msg&&<div className="alert">{msg}</div>}</div>}
...
function App(){return <Layout><Routes><Route path="/" element={<Home/>}/><Route path="/about" element={<About/>}/><Route path="/join" element={<Join/>}/><Route path="/events" element={<EventsPage/>}/><Route path="/donate" element={<Donate/>}/><Route path="/archive" element={<Archive/>}/><Route path="*" element={<NotFound/>}/></Routes></Layout>}
